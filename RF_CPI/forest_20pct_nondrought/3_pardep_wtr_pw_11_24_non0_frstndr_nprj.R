# CPI Step 3: partial dependence — Forest watersheds, Non-drought years, Hydropower (Power_diverted)
# Run as SLURM array job: --array=1-11 (focal predictors only; year_wtr/month excluded)
...
load(paste0("rf_pw_all_huc8_non0_6_1000_pct_frstndr_11_24_nprj.RData"))
...
for (i in num) {
  ...
  setwd(paste0(input.dir, "output/pardep/frstndr/"))

  write.csv(pd.pw,
            paste0("pardep_rf_pw_", var.name, "_huc8_6_1000_non0_pct_frstndr_11_24_nprj.csv"),
            row.names = FALSE)
