...
wtr.data.sub.non0 <- wtr.data %>% group_by(huc8, year) %>% filter(sum(Power_diverted) >= 1000)
#wtr.data.sub3.non0 <- subset(wtr.data.sub2.non0, Power_diverted > 0)
wtr.data.sub.non0 <- wtr.data.sub.non0 %>% group_by(huc8, year) %>% filter(mean(Forest_pct) < 20)

# drought years only; 2012-2015 & 2020-2022
wtr.data.sub.non0 <- subset(wtr.data.sub.non0, year_wtr %in% c(2011, 2016, 2017, 2018, 2019, 2023))
...
#max(wtr.data.sub2.non0$BurnSev4_10yr_pct)
...
rf.pw <- cforest(Power_diverted ~ mng_medhigh_10yr_pct + BurnSev4_10yr_pct + et_mean + tmean + prcp_sum + swe_mean + inflow_wtr_mm + sum_cap_af + elevation + pop_den + weighted_median_income + project + year_wtr + month,
                 data = wtr.data.sub2.non0,
                 control = cforest_unbiased(mtry = mtry.num, 
                                            # default 5 "for merely technical reasons"
                                            ntree = ntree.num) ) # sqrt(n) or n/3
# save Rdata
setwd(paste0(input.dir,"input"))
save(rf.pw, file = paste0("rf_pw_all_huc8_non0_",mtry.num,"_",ntree.num,"_pct_nfrstndr.RData"))

print("pw non-Forest-drought years, Cforest done")
