# CPI Step 2: permimp — Non-forest watersheds, Non-drought years, Consumptive (consumtive_diverted)
...
load(paste0("rf_con_all_huc8_non0_6_1000_pct_nfrstndr_11_24.RData"))

set.seed(160617)

## Conditional Permutation Importance
imp.con <- permimp(rf.con,
                   conditional = TRUE,
                   threshold   = thrd,
                   progressBar = TRUE)
...
## information about how consistently a variable contributes across the forest.
imp.con.perdf <- as.data.frame(imp.con$perTree)

## Melt to long format
imp.con.perdf.m <- melt(imp.con.perdf,
...
write.csv(imp.con.perdf,
          paste0("permimp_cond_rf_con_all_huc8_6_1000_thrld", thrd.name,
                 "_non0_raw_pct_nfrstndr_11_24.csv"),
          row.names = FALSE)

write.csv(imp.con.perdf.m.ci,
          paste0("permimp_cond_rf_con_all_huc8_6_1000_thrld", thrd.name,
                 "_non0_pct_nfrstndr_11_24.csv"),
          row.names = FALSE)

write.csv(imp.con.df.nm,
          paste0("permimp_cond_avg_rf_con_all_huc8_6_1000_thrld", thrd.name,
                 "_non0_pct_nfrstndr_11_24.csv"),
          row.names = FALSE)
