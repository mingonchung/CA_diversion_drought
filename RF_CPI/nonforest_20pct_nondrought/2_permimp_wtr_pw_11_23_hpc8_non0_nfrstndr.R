...
load(paste0("rf_pw_all_huc8_non0_4_1000_pct_nfrstndr.RData"))

set.seed(160617)
# variable importance
# threshold should be larger than 0.8
imp.pw <- permimp(rf.pw, conditional = TRUE, 
                  threshold=thrd, 
                  progressBar=TRUE)
...
imp.pw.perdf[imp.pw.perdf==0] <- NA

# melt
imp.pw.perdf.m <- melt(imp.pw.perdf, variable.name="Feature",
                       value.name = "value")
...
# plot(imp.pw, type = "bar", interval = "sd") #quantile

print("Varimp done")

# write.csv
setwd(paste0(input.dir,"output/varimp/nfrstndr/"))

write.csv(imp.pw.perdf,  paste0("permimp_cond_rf_pw_all_huc8_4_1000_thrld",thrd.name,"_non0_raw_pct_nfrstndr.csv"), row.names=F)

write.csv(imp.pw.perdf.m.ci,  paste0("permimp_cond_rf_pw_all_huc8_4_1000_thrld",thrd.name,"_non0_pct_nfrstndr.csv"), row.names=F)

write.csv(imp.pw.df.nm,  paste0("permimp_cond_avg_rf_pw_all_huc8_4_1000_thrld",thrd.name,"_non0_pct_nfrstndr.csv"), row.names=F)
