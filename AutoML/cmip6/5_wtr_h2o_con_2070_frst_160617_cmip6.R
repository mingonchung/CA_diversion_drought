...
for(a in 1:length(GCM.list)) {
  ...
  # Investigate test error
  simul_h2o_pred_list <- pred_wtr %>%
    filter(year_wtr > 2014) %>%
    select(year_wtr, 1:6) %>%
    tibble::add_column(
      pred = simul_h2o_pred %>% as.tibble() %>% pull(predict)
    )
  ...
  write.csv(simul_h2o_pred_list, paste0("h2o_con_pred_huc8_",GCM.list[[a]],"_",num,"_frst.csv"), row.names=F)
  write.csv(simul_h2o_pred_list_yr, paste0("h2o_con_pred_huc8_yr_",GCM.list[[a]],"_",num,"_frst.csv"), row.names=F)
