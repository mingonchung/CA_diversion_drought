...
wtr.data.sub.non0 <- wtr.data.sub %>% group_by(huc8, year) %>% filter(sum(consumtive_diverted) >= 1000)
wtr.data.sub.non0 <- subset(wtr.data.sub.non0, consumtive_diverted > 0)
...
#summary(automl.h2o.con) #conf.int=0.95
...
error_wtr <- wtr.data.sub2.non0[, c("huc8", "year", "month", "year_wtr", "consumtive_diverted")] %>%
...
error_wtr_train <- wtr.data.sub2.non0[, c("huc8", "year", "month", "year_wtr", "consumtive_diverted")] %>%
...
error_wtr_yr_train <- error_wtr_train %>%
  ...
  dplyr::summarize(n=n(),
                   ...
                   pred_sum = sum(pred))
## write.csv
output.dir <- paste0(input.dir,"output/prediction/",year.sel,"_",working.hr,"hr/cmip5/")
...
write.csv(perf_summary, paste0("h2o_con_perf_test_huc8_",num,"_frst.csv"), row.names = FALSE)
write.csv(error_wtr, paste0("h2o_con_all_predict_huc8_",num,"_frst.csv"), row.names=F)
write.csv(error_wtr_yr, paste0("h2o_con_error_predict_huc8_",num,"_frst.csv"), row.names=F)
write.csv(error_wtr_yr_train, paste0("h2o_con_error_predict_huc8_",num,"_frst_train.csv"), row.names=F)
write.csv(error_wtr_sum, paste0("h2o_con_error_summary_huc8_",num,"_frst.csv"), row.names=F)
...
## different projections #./projection/
...
for(a in 1:length(GCM.list)) {
  ...
  for(numi in 1:4) {
    ...
    wtr.pred <- read.csv(paste0("CA_wtr_HUC8_all_",list.prj[[numi]],"_",GCM.list[[a]],".csv"), header=T, stringsAsFactors=FALSE)
    ...
    # Investigate test error
    simul_h2o_pred_list <- pred_wtr[,c(1:5)] %>%
      filter(year_wtr > 2010) %>%
    ...
    write.csv(simul_h2o_pred_list, paste0("h2o_con_pred_huc8_",list.prj[[numi]],"_",GCM.list[[a]],"_",num,"_frst.csv"), row.names=F)
    write.csv(simul_h2o_pred_list_yr, paste0("h2o_con_pred_huc8_yr_",list.prj[[numi]],"_",GCM.list[[a]],"_",num,"_frst.csv"), row.names=F)
