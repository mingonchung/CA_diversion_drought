...
for(a in 1:length(GCM.list)) {
  ...
  for(numi in 1:4) {
    ...
    write.csv(simul_h2o_pred_list, paste0("h2o_pw_pred_huc8_",list.prj[[numi]],"_",GCM.list[[a]],"_",num,"_frst.csv"), row.names=F)
    write.csv(simul_h2o_pred_list_yr, paste0("h2o_pw_pred_huc8_yr_",list.prj[[numi]],"_",GCM.list[[a]],"_",num,"_frst.csv"), row.names=F)
