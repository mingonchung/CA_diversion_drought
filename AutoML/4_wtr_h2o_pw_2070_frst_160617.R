...
# H2O AutoML - Hydropower diversion projection, forested CA HUC8s (prefix 18)
...
#     training range 2011-2024; year labels caused memorisation (train R2=0.966
#     vs test R2=0.212). Drought state encoded through physical variables only.
...
# -----------------------------------------------------------------------
huc8_eligible_pw <- wtr.data %>%
  group_by(huc8) %>%
  summarize(cum_div = sum(Power_diverted, na.rm = TRUE), .groups = "drop") %>%
  filter(cum_div >= 1000) %>%
  pull(huc8)

wtr.sub <- wtr.data %>%
  filter(huc8 %in% huc8_eligible_pw,
...
#    3 of 14 years = 21%, covers full hydrological state space
# -----------------------------------------------------------------------
...
# H2O perf metrics on raw scale. Original-scale R2/RMSE/NSE/PBIAS
# computed manually from predictions for consistency with con script.
# -----------------------------------------------------------------------
...
# In Section 7, change your evaluation dataframes:
error_wtr <- wtr.sub[, c("huc8","year","month","year_wtr","Power_diverted")] %>%
  filter(year_wtr %in% test_years) %>%
  add_column(pred = as_tibble(pred_h2o)$predict) %>%
  rename(actual = Power_diverted) %>%
  mutate(pred      = pmax(pred, 0), # Still ensure no negative predictions
         error     = actual - pred,
         error_pct = if_else(actual != 0, error / actual, NA_real_))

error_wtr_train <- wtr.sub[, c("huc8","year","month","year_wtr","Power_diverted")] %>%
  filter(!year_wtr %in% test_years) %>%
...
perf_summary <- data.frame(
  # Original-scale metrics -- use these in manuscript
  ...
  R2_test_h2o    = h2o.r2(perf_test),   RMSE_test_h2o  = h2o.rmse(perf_test),
  R2_train_h2o   = h2o.r2(perf_train),  RMSE_train_h2o = h2o.rmse(perf_train),
...
write.csv(perf_summary,       paste0("h2o_pw_perf_huc8_",     num, "_frst.csv"), row.names=FALSE)
write.csv(error_wtr,          paste0("h2o_pw_all_predict_",   num, "_frst.csv"), row.names=FALSE)
write.csv(error_wtr_yr,       paste0("h2o_pw_error_yr_",      num, "_frst.csv"), row.names=FALSE)
write.csv(error_wtr_yr_train, paste0("h2o_pw_error_yr_train_",num, "_frst.csv"), row.names=FALSE)
write.csv(error_wtr_sum,      paste0("h2o_pw_error_summary_", num, "_frst.csv"), row.names=FALSE)
...
for (a in seq_along(GCM.list)) {
  for (numi in seq_along(list.prj)) {
    ...
    pred_list  <- pred_wtr[, 1:5] %>%
      add_column(pred = pmax(as_tibble(simul_pred)$predict, 0))
    pred_list_yr <- pred_list %>%
      group_by(date) %>% summarize(n=n(), pred_sum=sum(pred), .groups="drop")
    setwd(cmip5.dir)
    write.csv(pred_list,
      paste0("h2o_pw_pred_",    list.prj[numi],"_",GCM.list[a],"_",num,"_frst.csv"), row.names=FALSE)
    write.csv(pred_list_yr,
      paste0("h2o_pw_pred_yr_", list.prj[numi],"_",GCM.list[a],"_",num,"_frst.csv"), row.names=FALSE)
    h2o.rm(h2o_pred)
    h2o.rm(simul_pred)
...
for (a in seq_along(GCM6.list)) {
  ...
  setwd(cmip6.dir)
  write.csv(pred_list,
    paste0("h2o_pw_pred_",    GCM6.list[a],"_",num,"_frst.csv"), row.names=FALSE)
  write.csv(pred_list_yr,
    paste0("h2o_pw_pred_yr_", GCM6.list[a],"_",num,"_frst.csv"), row.names=FALSE)
  h2o.rm(h2o_pred)
  h2o.rm(simul_pred)
